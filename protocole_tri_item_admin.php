			...
			<div class="page">
				...
				<table class="table" style="font-size:10px;text-align:center; width: 900px; float:left; margin-top: 40px;">
					...
					<?php
						...
						if(!$bool)
						{
							if(isset($_POST['ferraille'])){$sql.=" WHERE Categorie like Ferraille";}
							if(isset($_POST['musee'])){$sql.=" WHERE Categorie like Musée";}
							if(isset($_POST['vip'])){$sql.=" WHERE Categorie like VIP";}
							if(isset($_POST['enchere'])){$sql.=" WHERE TypeVente like Enchères";}
							if(isset($_POST['immediat'])){$sql.=" WHERE Categorie like Vente directe";}
							if(isset($_POST['nego'])){$sql.=" WHERE Categorie like Négociation possible";}
							/*if(isset($_POST['prix_min'])){
								$prix_min = $_POST['prix_min'];
								$sql .=" WHERE Prix >= '$prix_min'";
							}
							if(isset($_POST['prix_max'])){
								$prix_max = $_POST['prix_max'];
								$sql .=" WHERE Prix <= '$prix_max'";
							}*/
							$bool=true;
						}
						if($bool)
						{
							if(isset($_POST['ferraille'])){$sql.=" AND Categorie like Ferraille";}
							if(isset($_POST['musee'])){$sql.=" AND Categorie like Musée";}
							if(isset($_POST['vip'])){$sql.=" AND Categorie like VIP";}
							if(isset($_POST['enchere'])){$sql.=" AND TypeVente like Enchères";}
							if(isset($_POST['immediat'])){$sql.=" AND Categorie like Vente directe";}
							if(isset($_POST['nego'])){$sql.=" AND Categorie like Négociation possible";}
							/*if(isset($_POST['prix_min'])){
								$prix_min = $_POST['prix_min'];
